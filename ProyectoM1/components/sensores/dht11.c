...
int dht11_read(dht11_t *dht11,int connection_timeout)
{
    ...
    while(timeout_counter < connection_timeout)
    {
        ...
        if(waited == -1) { ESP_LOGE("DHT11:","Failed at phase 1"); ets_delay_us(20000); continue; } 

        waited = wait_for_state(dht11,1,90);
        if(waited == -1) { ESP_LOGE("DHT11:","Failed at phase 2"); ets_delay_us(20000); continue; } 
        
        waited = wait_for_state(dht11,0,90);
        if(waited == -1) { ESP_LOGE("DHT11:","Failed at phase 3"); ets_delay_us(20000); continue; } 
    ...
    int crc = (received_data[0] + received_data[1] + received_data[2] + received_data[3]) & 0xff;
